...
router.get('/chat/allMessages', (req: any, res: any) => {
  res.status(req.statusCode || 200)
    .json({zone: 'Chat all messages'});
});

router.get('/chat/addUser', (req: any, res: any) => {
  res.status(req.statusCode || 200)
    .json({zone: 'Chat add user'});
});

router.get('/chat/removeUser', (req: any, res: any) => {
  res.status(req.statusCode || 200)
    .json({zone: 'Chat remove user'});
});

router.get('/analytics/:actionName?/:clientId?/:eventId?', async (req: any, res: any) => {
  const doTracking: any = await analytics.set(req);
  res.status(req.statusCode || 200)
    .json(doTracking);
});

/*router.get('/truechat', (req: any, res: any) => {
  let htmPath: string = config.appVars.rootDir + 'src\\demos\\truechat.htm';
